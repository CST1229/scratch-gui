...
const getScaleTool = tool => tool.boundingBoxTool._modeMap.SCALE;
...
export const updateScaleTool = (paper, tool) => {
    ...
    const axisLineX = new paper.Path.Line({
        from: [0, 0],
        to: [0, 0],
        strokeColor: new paper.Color(guideColor),
        strokeWidth: 1 / paper.view.zoom,
    ...
    const axisLineY = new paper.Path.Line({
        from: [0, 0],
        to: [0, 0],
        strokeColor: new paper.Color(guideColor),
        strokeWidth: 1 / paper.view.zoom,
    ...
    addons.paintSnap = function (event) {
        ...
        const canSnapXCoords = Object.entries(snapXCoords).map(format)
            .filter(canSnap)
            .sort(sortByPrioOrDist);

        const snapYCoords = createScalePoints(paper, lib, paintLayer.children, false, doesSy);

        const canSnapYCoords = Object.entries(snapYCoords).map(format)
            .filter(canSnap)
            .sort(sortByPrioOrDist);
